    ...
    public static void main(String[] args) {
        ...
        for (Tester each : testers) {
            if(each.getSalary()>maxSalaryTester.getSalary()){
                ...
                System.out.println("Tester with maximum salary: "+maxSalaryTester);


        //   2.5 which developer has the maximum salary?

        Developer maxSalaryDev = developers.get(0);
        for (Developer each : developers) {
            if(each.getSalary()>maxSalaryDev.getSalary()){
                maxSalaryDev=each;
            }
        }
        System.out.println("Developer with maximum salary: "+maxSalaryDev);
